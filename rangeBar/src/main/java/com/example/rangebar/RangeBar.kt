...
class RangeBar @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
 ...
 private val isRoundToMin = false
 ...
 private fun inputsFromEditText():Boolean{
  return try {
   var currentTxt = binding.et.text?.toString()?.toInt() ?: 0
 ...
 private fun triggerCallBack(){
  try {
   onRangeChanged?.onChange(binding.et.text.toString().toInt())
  }catch (e:Exception){
   e.printStackTrace()
